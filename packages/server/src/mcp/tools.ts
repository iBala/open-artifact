 ...
 *
 * Every description is written "this connection published" because that is the
 * default scope; under MCP_ARTIFACT_SCOPE=user it is reworded to "you
 * published" rather than maintained as a second copy of each string, since the
 * two modes are the same tools with a different reach. The titles and the flags
 * say nothing about scope, so neither needs a second version.
 */
export function listMcpTools(config: Config): {
  name: string;
  title: string;
  description: string;
  inputSchema: Record<string, unknown>;
