...
const TOOLS = listMcpTools(configWith());
...
describe('what the flags claim, tool by tool', () => {
  ...
  it('says the same thing about behaviour whatever the artifact scope', () => {
    // MCP_ARTIFACT_SCOPE changes how far a tool can reach, not what it does when
    // it gets there. Widening the scope must not quietly relabel a reader as a
    // writer, or move a title, so the flags are compared across both modes.
    const widened = listMcpTools(configWith({ MCP_ARTIFACT_SCOPE: 'user' }));
    for (const tool of TOOLS) {
      const same = widened.find((other) => other.name === tool.name);
      expect(same?.title, tool.name).toBe(tool.title);
      expect(same?.annotations, tool.name).toEqual(tool.annotations);
    }
    // The descriptions are the part that does change, and only in that one mode.
    expect(TOOLS.some((tool) => tool.description.includes('this connection published'))).toBe(true);
    expect(widened.some((tool) => tool.description.includes('this connection published'))).toBe(
      false,
    );
  });
