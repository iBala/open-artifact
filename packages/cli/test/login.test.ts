 ...
 *
 * Nothing about the network, the emailed code or the token is stubbed. Signing in
 * is two runs: one that mails a code, one that spends it. That is exactly the
 * shape a person or an assistant follows, and it never blocks waiting for input.
 */
...
/** The six-digit code an instance just emailed to an address. */
function emailedCode(inst: TestInstance, email: string): string {
  const message = inst.mailer.lastTo(email);
